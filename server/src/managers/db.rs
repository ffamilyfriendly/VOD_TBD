...
pub fn ensure_tables() -> Result<(), rusqlite::Error> {
    ...
    // Source / upload
    con.execute("CREATE TABLE IF NOT EXISTS sources (source_id UUID PRIMARY KEY, url TEXT, type TEXT, priority INTEGER DEFAULT 1, size INTEGER, parent TEXT NOT NULL, uploaded_by INTEGER NOT NULL, video_codec TEXT, audio_codec TEXT, FOREIGN KEY(uploaded_by) REFERENCES users(id))", ())?;
    con.execute("CREATE TABLE IF NOT EXISTS uploads (source_id UUID PRIMARY KEY, total_bytes INTEGER NOT NULL, bytes_uploaded INTEGER DEFAULT 0, last_push INTEGER NOT NULL, filetype TEXT NOT NULL, FOREIGN KEY(source_id) REFERENCES sources(source_id))", ())?;
    Ok(())
}